...
impl egui::Plugin for WebA11y {
    ...
    fn output_hook(&mut self, ctx: &egui::Context, output: &mut egui::FullOutput) {
        ...
        // the top when it is discarded; egui-reactor's taffy layout normally
    ...
    impl Mirror {
        ...
        pub(super) fn attach(&mut self, ctx: &egui::Context) {
            ...
            let Some(canvas) = canvas else {
                log::error!(
                    "egui-reactor a11y: no <canvas id=\"{}\"> to mirror",
                    self.canvas_id
                );
                return;
            };
            let Some(parent) = canvas.parent_element() else {
                log::error!("egui-reactor a11y: the canvas has no parent element");
                return;
            };
            ...
            let Some(mut adapter) = Adapter::new(&parent, &canvas, NoActivation, queue) else {
                log::error!("egui-reactor a11y: no document to build the mirror in");
                return;
            };

...
//! against the definitions and falls back (see `egui_reactor_elements`). Bytes
...
///
/// ```
/// use egui_reactor_app::fonts::{FontSource, Generic, parse_css};
...
///
/// `egui-reactor-app`'s `default_fonts` feature is on by default and is what
...
impl Fonts {
    ...
    pub fn css(self, name: impl Into<Arc<str>>, spec: &str) -> Self {
        ...
        let chain = match parse_css(spec) {
            Ok(chain) => chain,
            Err(err) => {
                log::warn!("egui-reactor fonts: stack {name:?}: {err}");
                Vec::new()
            }
        };

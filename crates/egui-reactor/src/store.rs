...
fn collision_message(location: &Location<'static>) -> String {
    format!(
        "egui-reactor: hook id collision at {}:{}:{}. Wrap custom hooks in #[hook], or add key= inside loops.",
        location.file(),
        location.line(),
        location.column(),
    )
...
pub(crate) fn persisted_id(key: &str) -> egui::Id {
    egui::Id::new(("egui_reactor_persisted", key))
}
...
impl Store {
    ...
    fn show_collision_overlay(&self) {
        ...
        egui::Area::new(egui::Id::new("egui_reactor_collision_warning"))
    ...
    pub(crate) fn slot(
        ...
        if let Some(slot) = self.slots.get(&id) {
            if slot.last_visited.get() == pass {
                ...
                log::warn!("egui-reactor: hook id collision at {location} (id {id:?})");
            }
        ...
        if let Some(slot) = self.slots.get(&id) {
            if slot.last_visited.get() == pass {
                self.collisions
                    .borrow_mut()
                    .push(Collision { id, location });
                log::warn!("egui-reactor: use_persisted key collision at {location} (key {key:?})");
            }
            slot.last_visited.set(pass);
            return slot;
        }
    ...
    pub fn load_persisted(&mut self, json: &str) {
        match serde_json::from_str::<HashMap<String, String>>(json) {
            ...
            Err(err) => log::warn!("egui-reactor: could not read persisted state: {err}"),
        }
    }
    ...
    pub fn save_persisted(&self) -> String {
        ...
        serde_json::to_string(&*self.persisted.borrow()).unwrap_or_else(|err| {
            log::warn!("egui-reactor: could not write persisted state: {err}");
            String::from("{}")
        })
    }
}

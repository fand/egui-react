//! Core of egui-reactor: `View`, `Cx`, `Store`, `State`, `Handle`, `Dispatch`,
//! the hooks and the layout and paint attribute types.
//!
//! `rsx!`, `#[component]` and `#[hook]` come from `egui-reactor-macros` and are
//! re-exported here, so `use egui_reactor::prelude::*` is the only import a
...
pub use egui_reactor_macros::{component, hook, rsx};

...
//! `crates/egui-reactor/tests/lite_parity.rs` checks node by node.
//!
...
impl LiteTree {
    ...
    fn fall_back(&mut self, field: &'static str) {
        if self.fallback.is_none() {
            ...
            log::debug!(
                "egui-reactor: a row falls back to the taffy layout path: `{field}` is outside the lite subset"
            );
        }
    }
...
// which egui-reactor's styles cannot express; and `box-sizing: border-box`,
...
impl LiteCx<'_> {
    ...
    fn fall_back(&mut self, field: &'static str) {
        ...
        self.root_ui
            .ctx()
            .request_discard("egui-reactor: row layout falls back to taffy");
    }
...
pub(crate) fn show<R>(
...

    {
        ...
        if let Some(reason) = discard {
            log::debug!("egui-reactor: request_discard: {reason}");

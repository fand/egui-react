...
//!
//! This is what egui-reactor uses instead of `egui_taffy`. The behaviour is the
    ...
    fn add_child_node(
        ...
        let node = match nodes.entry(key) {
            Entry::Occupied(mut entry) => {
                ...
                if slot.keep {
                    log::error!("egui-reactor: two layout nodes share the id {key:?}");
                }
                slot.keep = true;
                let node = slot.node;
...
/// the engine logs it at `debug` (`RUST_LOG=egui_reactor=debug`).
fn discard_reason(
    what: &str,
    created: bool,
    removed: bool,
    ...
    let mut reason = format!("egui-reactor: {what} changed:");
    if created {

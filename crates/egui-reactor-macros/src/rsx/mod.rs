    ...
    fn node(&mut self, node: &Node<ControlFlow>) -> TokenStream {
        match node {
            ...
            Node::Text(text) => {
                ...
                quote! { ::egui_reactor::View::show(#value, cx); }
            }
            Node::Block(block) => {
                let expr = block_expr(block);
                quote! { ::egui_reactor::View::show(#expr, cx); }
            }
            ...
            Node::RawText(raw) => {
                self.errors.push(error_at(
                    raw.span(),
                    "egui-reactor: text must be a string literal: \"...\"",
                ));
                quote! {}
            }
            Node::Doctype(doctype) => {
                self.errors.push(error_at(
                    doctype.span(),
                    "egui-reactor: <!DOCTYPE> has no meaning in rsx!",
                ));
                quote! {}
            }
    ...
    fn element(&mut self, element: &NodeElement<ControlFlow>) -> TokenStream {
        ...
        let path = match element.name() {
            ...
            other => {
                return error_at(
                    other.span(),
                    "egui-reactor: an element name must be a component path, like <Button/> or \
                     <elements::Button/>",
                );
            }
        };
        ...
        quote_spanned! { span =>
            ::egui_reactor::__private::enter_scope(
                #cx,
                #source,
                ::egui_reactor::props_builder(&#path)
                    #(#setters)*
                    #style
                    #events
                    .children(#children)
    ...
    fn children(&mut self, children: &[Node<ControlFlow>]) -> TokenStream {
        match children {
            ...
            nodes => {
                ...
                quote! { ::egui_reactor::view(|cx| { #body }) }
            }
        }
    }
}
...
impl Attributes {
    fn parse(element: &NodeElement<ControlFlow>, path: &syn::ExprPath) -> syn::Result<Self> {
        ...
        for attr in element.attributes() {
            let NodeAttribute::Attribute(attr) = attr else {
                return Err(syn::Error::new(
                    attr.span(),
                    "egui-reactor: block attributes (`<Name {expr}/>`) are not supported",
                ));
            };
            let NodeName::Path(key_path) = &attr.key else {
                return Err(syn::Error::new(
                    attr.key.span(),
                    "egui-reactor: an attribute name must be a plain identifier",
                ));
            };
            ...
            if seen.contains(&text) {
                return Err(syn::Error::new(
                    name.span(),
                    format!("egui-reactor: duplicate attribute `{text}`"),
                ));
            }
            ...
            let value = match &attr.possible_value {
                ...
                KeyedAttributeValue::Value(value) => match &value.value {
                    ...
                    invalid => {
                        return Err(syn::Error::new(
                            invalid.span(),
                            "egui-reactor: this attribute value is not a valid expression",
                        ));
                    }
                },
                KeyedAttributeValue::Binding(binding) => {
                    return Err(syn::Error::new(
                        ...
                        "egui-reactor: closure-binding attributes are not supported",
                    ));
                }
            };
        ...
        if out.events.is_some() && !out.handlers.is_empty() {
            return Err(syn::Error::new(
                element.name().span(),
                "egui-reactor: `events=` replaces the fused `on_*` closure; use one or the other",
            ));
        }
        Ok(out)
    }
    ...
    fn style_call(&self) -> Option<TokenStream> {
        let base = match &self.style {
            ...
            None => quote!(::egui_reactor::layout::ItemStyle::default()),
        };
    ...
    fn events_call(&self) -> Option<TokenStream> {
        ...
        let arms = self.handlers.iter().map(|(enum_path, variant, handler)| {
            quote_spanned! { variant.span() =>
                #enum_path::#variant(__egui_reactor_payload) => {
                    ::egui_reactor::Handler::call(#handler, __egui_reactor_payload)
                }
            }
        });
        Some(quote! {
            .events(&mut |__egui_reactor_event| {
                #[allow(unreachable_patterns)]
                match __egui_reactor_event {
                    #(#arms)*
                    _ => {}
                }
            })

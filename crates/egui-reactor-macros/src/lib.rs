//! Procedural macros for egui-reactor: `rsx!`, `#[component]` and `#[hook]`.
//!
//! These are re-exported from `egui_reactor`, which is also the crate the
//! expansions refer to: everything they emit is fully qualified as
//! `::egui_reactor::..`, so user code only needs `use egui_reactor::prelude::*`.
...
/// Turn a function into a egui-reactor component.
///
/// ```ignore
/// #[component]
...
/// ```
///
/// Expands to `::egui_reactor::view(|cx| { .. })`. Elements are component

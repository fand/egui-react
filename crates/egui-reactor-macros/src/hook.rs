...
fn try_expand(attr: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    if !attr.is_empty() {
        return Err(syn::Error::new_spanned(
            attr,
            "egui-reactor: #[hook] takes no arguments",
        ));
    }
    ...
    let cx = function
        ...
        .ok_or_else(|| {
            syn::Error::new(
                function.sig.span(),
                "egui-reactor: a #[hook] function must take a `&mut Cx` argument; \
    ...
    function.block = syn::parse_quote!({
        let __egui_reactor_location = ::std::panic::Location::caller();
        #cx.hook_scope(__egui_reactor_location, |#cx| #body)
    });

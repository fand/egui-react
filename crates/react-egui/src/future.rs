...
mod task {
    ...
    pub(crate) fn spawn(fut: impl Future<Output = ()> + Send + 'static) {
        if let Err(err) = std::thread::Builder::new()
            .name(String::from("react-egui-future"))
            .spawn(move || pollster::block_on(fut))
        {
            ...
            log::error!("react-egui: could not spawn a thread for a future: {err}");
        }
    }
    ...
    F: SpawnFuture<T>,
{
    ...
    let inbox_slot = store.slot(id.with("__react_egui_future_inbox"), location, || {
        Box::new(Inbox::<T> {

//! Procedural macros for egui-react: `rsx!`, `#[component]` and `#[hook]`.
//!
//! These are re-exported from `egui_react`, which is also the crate the
//! expansions refer to: everything they emit is fully qualified as
//! `::egui_react::..`, so user code only needs `use egui_react::prelude::*`.
...
/// Turn a function into a egui-react component.
///
/// ```ignore
/// #[component]
...
/// ```
///
/// Expands to `::egui_react::view(|cx| { .. })`. Elements are component

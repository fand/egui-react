...
fn try_expand(attr: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    if !attr.is_empty() {
        return Err(syn::Error::new_spanned(
            attr,
            "react-egui: #[hook] takes no arguments",
        ));
    }
    ...
    let cx = function
        ...
        .ok_or_else(|| {
            syn::Error::new(
                function.sig.span(),
                "react-egui: a #[hook] function must take a `&mut Cx` argument; \
    ...
    function.block = syn::parse_quote!({
        let __react_egui_location = ::std::panic::Location::caller();
        #cx.hook_scope(__react_egui_location, |#cx| #body)
    });

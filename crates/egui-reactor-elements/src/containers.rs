...
use egui_reactor::layout::{ItemStyle, Length, root_style};
...
impl From<&str> for Side {
    ...
    fn from(s: &str) -> Self {
        Self::parse(s).unwrap_or_else(|| {
            panic!("egui-reactor: {s:?} is not a valid Side (expected left, right, top or bottom)")
        })
    }
}
...
impl From<&str> for Anchor {
    ...
    fn from(s: &str) -> Self {
        Self::parse(s).unwrap_or_else(|| {
            panic!(
                "egui-reactor: {s:?} is not a valid Anchor (expected top-left, top, top-right, \
...
impl From<&str> for Order {
    ...
    fn from(s: &str) -> Self {
        Self::parse(s).unwrap_or_else(|| {
            panic!(
                "egui-reactor: {s:?} is not a valid Order (expected background, middle, foreground \
                 or tooltip)"
            )
        })
    }

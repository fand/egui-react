...
//!   built on egui-reactor's own layout engine over taffy (`cx.container`,
//!   `cx.text`).
...
//! [`ItemStyle`](egui_reactor::ItemStyle) through a `style` prop, which `rsx!`
//! fills in from `w=` / `grow=` / `p=` and friends. The same prop carries the
//! [`PaintStyle`](egui_reactor::PaintStyle) — `bg` `border` `radius` `shadow`

//! Runs the plain egui board on its own, next to `cargo run -p board`.
//!
//! `save` and `load` are written out here. The egui-reactor version gets the same
//! thing from `use_persisted("board/board", ..)`.

#[cfg(not(target_arch = "wasm32"))]
use board::plain::{self, PlainState, STORAGE_KEY};

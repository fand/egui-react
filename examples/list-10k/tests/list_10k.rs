...
#[test]
fn the_filter_narrows_the_egui_reactor_list() {
...
/// Removing a row takes one off the count. Only the egui-reactor version is
...
/// Only the rows in view are built: the first row is in the tree, the last is
/// not.
#[test]
fn only_the_rows_in_view_exist() {
    let mut harness = react();
    harness.run();
...
/// The two versions show the same rows at the top of the list.
#[test]
fn both_versions_show_the_same_first_rows() {
    let mut react = react();
    react.run();
    let mut plain = plain();
    plain.run();
    ...
    for i in 0..10 {
        ...
        assert!(react.query_by_label(&label).is_some(), "{label}");
        assert!(plain.query_by_label(&label).is_some(), "{label}");
    }
}

...
egui-react が包んでいるのは egui の便利な一部分で、全部ではありませんし、全部である必要もありません。最後はすべて `&mut egui::Ui` なので、egui にできることは呼び出し 1 回の距離にあります。4 つの節が 4 つの道筋です。木の途中に普通のコードを書く `{view(|cx| ..)}`、いまいる場所に egui を描く`cx.leaf(&style, |ui| ..)`、自分で描くための「確保して描く」、そして egui のコンテナのクロージャの中でフックを使うための、内側の `Ui` に対する新しい `Cx`。
...
これは回避策ではありません。`egui-react-elements` の要素も同じ呼び出しで書かれています。裏口はありません。同じ 4 つをもう少し言葉で説明したものが[エスケープハッチ](/ja/guide/escape-hatches) です。

...
This was originally PR C in [docs/tasks/examples/](../examples/task.md). PR A (#4) / PR B (#6 -> #8) were merged first, and work on C stopped after only `Options.setup` was implemented, so the rest was split out as its own task. Core (`egui-reactor`, `egui-reactor-macros`) is not touched.

## Current state

- Done: `Options.setup: Option<Setup>` (`egui-reactor-app`). Called at the top of `ReactApp::new`. `wgpu = "30.0"` pinned in the workspace. ARCHITECTURE sections 7 / 8 updated.
...
- `<Canvas>` (`egui-reactor-elements`): `style` / `sense` / `on_paint` / `on_drag` / `on_hover`. taffy decides the size via `leaf_fill`. No dependency on egui-wgpu. With kittest.
...
- `crates/egui-reactor-elements/src/canvas.rs` + `tests/canvas.rs`.
- `examples/shader/`.

...
Make egui-reactor apps that run on the web (wasm) usable from assistive technology: screen readers and keyboard navigation. On native, egui passes the widget tree to the OS accessibility API through AccessKit. On the web, that tree is thrown away. Keep drawing on the canvas, and mirror only the accessibility tree into the DOM (the same approach as the semantics layer in Flutter web).

This is not an egui-reactor specific problem. It is an unfinished part of egui / AccessKit / eframe. So we assume the result goes upstream (an AccessKit web adapter, a hook in eframe). egui-reactor is in a position where "once it lands upstream, we get the benefit for free". This task builds a prototype to confirm the shape, and takes it as far as bringing it upstream.
...
egui-reactor elements use egui widgets as-is, so they get the native support for free (kittest finding elements by label is the proof).
...
- **Prototype (closed inside egui-reactor)**:
  ...
  - Target widgets: Button / Checkbox / Label / TextEdit / Slider / ComboBox (all widgets in egui-reactor-elements).
...
- **Work on the egui-reactor side** (what we can do now, independent of upstream):
...
- Prototype code (a forked eframe or our own runner, and a prototype adapter). Not merged into egui-reactor main; kept on a branch or in a separate repository.
...
- ~~A proposal for a web adapter and an eframe hook is submitted upstream (merging is not a condition).~~ Withdrawn on 2026-09-05. We do not submit upstream. We keep `accesskit-web` and `WebA11y` inside egui-reactor.
...
- Write the adapter independent of egui (it looks only at AccessKit's `TreeUpdate`), in the same shape as the other AccessKit adapters. Nothing specific to egui / egui-reactor goes in.
- Do the prototype closed inside the egui-reactor-app wasm runner. Once it works, split it out upstream. Do not add a forked eframe as a dependency of egui-reactor main.

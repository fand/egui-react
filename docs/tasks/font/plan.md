...
**(c) Local Font Access API.** `window.queryLocalFonts()` resolves to an array of `FontData { postscriptName, fullName, family, style, blob() }`. (The research and the first implementation said `navigator.fonts.query()`, the shape of the 2020 draft; the API that shipped in Chrome 103 is a method on `Window`, and `navigator.fonts` is undefined in every browser. Caught on the second web run, see section 5.) Facts checked:

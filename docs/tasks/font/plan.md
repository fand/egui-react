...
- kittest in `egui-react-app/tests/fonts.rs`: apply a `Fonts` with `Bundled(HACK_REGULAR)` as stack `"code"`, run a frame, assert `ctx.fonts(|f| f.definitions().families[&Name("code")])` starts with Hack's key and `has_glyphs` holds.
...
- `fonts/local.rs` (wasm only): `Fonts::request_local_fonts(&self) -> impl Future<Output = Result<usize, JsValue>>`: `Reflect::get(navigator, "fonts")` → `query()` → for each face whose `family()` equals a `System(name)` in any stack, one `blob()` per family → `array_buffer()` → `Vec<u8>` → `db.load_font_source` → re-`apply`. Returns how many families were filled. It is `async` and meant to be called from an `on_click` inside `spawn_local`, because of the user activation rule; the doc says so and the example shows it.
- `Fonts::local_fonts_available() -> bool` (`"fonts" in navigator`) and `Fonts::local_fonts_permission() -> impl Future<Output = Option<PermissionState>>` for the UI.
...
- On native these three are present and return `false` / `None` / `Ok(0)` so the example is one source file.
...
- **Emoji / color fonts.** Open until step 3. Whether epaint 0.36 draws COLR / sbix / CBDT is decided there and recorded here. The text renderer may be replaced later, so the design keeps this to one filter in `resolve`.
...
- **`<Text font>` lookup per pass.** Measured in step 3; the fallback is a per-pass cache.

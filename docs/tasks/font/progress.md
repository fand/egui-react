...
| 1 | `844cfe6` | `egui_reactor_app::fonts`: `Fonts` / `FontStack` / `FontSource::{Bundled, System, Url, Generic}`, `parse_css`, resolver over one `fontdb::Database` into `FontDefinitions`, `report()` | Every source only puts bytes into the database; chains are fontdb queries; egui defaults stay at the tail of every chain |
...
| Fix | `f8d078d` | **Local Font Access** is `window.queryLocalFonts()`, not `navigator.fonts.query()` (the 2020 draft the plan had carried). **Gallery** code pane galleys keyed by `egui_reactor::fonts_generation(ctx)`, new public fn | Found on the second web run: the button was disabled in Chrome; the source pane garbled after the web font arrived, the same stale-galley bug as the engine's, in the gallery's own per-line cache |
...
| | `1ee2396` | `default_fonts` feature on `egui-reactor-app` (on by default): egui and eframe taken without default features workspace-wide, eframe's other defaults re-added by name, `winit` a direct dependency to keep `winit/default`. Resolver takes its base `FontDefinitions` as input and pushes a built-in key only when the bytes are there; an empty chain is an empty family (zero glyphs, no panic). CI runs `cargo test -p egui-reactor-app --no-default-features` | Cargo unifies features over the graph, so before this an app could not drop the 1.4 MB. Verified: `FontDefinitions::empty()` lays text out with zero glyphs in egui 0.36; the two panics are an unbound family and a key without data, both now guarded by a unit test against an empty base |

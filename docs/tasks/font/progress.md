...
| | `1ee2396` | `default_fonts` feature on `egui-react-app` (on by default): egui and eframe taken without default features workspace-wide, eframe's other defaults re-added by name, `winit` a direct dependency to keep `winit/default`. Resolver takes its base `FontDefinitions` as input and pushes a built-in key only when the bytes are there; an empty chain is an empty family (zero glyphs, no panic). CI runs `cargo test -p egui-react-app --no-default-features` | Cargo unifies features over the graph, so before this an app could not drop the 1.4 MB. Verified: `FontDefinitions::empty()` lays text out with zero glyphs in egui 0.36; the two panics are an unbound family and a key without data, both now guarded by a unit test against an empty base |

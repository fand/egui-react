...
| `/api/` | `cargo doc --no-deps` for the four public crates, plus a redirect to `egui_react/`, which rustdoc does not write | cargo |

- **The example pages are the gallery, redistributed.** The list is the sidebar, the code pane is the page body (with the egui-react / plain egui tabs where a plain version exists), and the running example is an iframe on `/embed/`. The native gallery (`cargo run -p gallery <name>`) is unchanged and stays the development tool.
- **One wasm for every example.** `examples/gallery/src/embed.rs` is a second bin in the gallery crate, so it reuses the example imports and `EXAMPLES`. It reads `location.hash` every pass, mounts `<Running>` under `key={name}` in a grow column, and registers a `hashchange` listener that calls `request_repaint` — egui repaints on input, and a hash change is not input. Unknown or missing names fall back to `EXAMPLES[0]`, and `plain` for an example without one falls back to the egui-react version, the same rule the gallery follows.

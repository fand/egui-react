...
`rsx!` always emits `::egui_reactor::view(|cx| { .. })`. `pub fn view<F: FnOnce(&mut Cx<'_, '_>)>(f: F) -> impl View` is a helper that exists only to pin the closure's argument type; writing a bare closure in an `impl View` position sometimes fails to infer the type of `cx`. Users also use `view(|cx| ..)` when writing an escape hatch.
...
The tail expression of the body is rewritten to `::egui_reactor::View::show(tail, cx)`. Wrapping the whole body as `View::show({ body }, cx)` does not compile, because the guards would still borrow the block's locals when returned, so only the tail expression is replaced. A body that returns a different `rsx!` per `if` / `match` arm has mismatched closure types, so write it as `rsx!{ if .. }`.

`<Counter initial={0} />` expands to the following.

```rust
cx.scope((file!(), line!(), column!(), 3usize, key), |cx| {
    Counter(cx, ::egui_reactor::props_builder(&Counter).initial(0).children(()).build());
});
```
...
The reason passed to `request_discard` names the cause (`egui-reactor: layout changed: the text "last frame 9.8 ms…" moved [..] -> [..]`). egui draws the reasons in its `PERF WARNING` overlay when discards run for three frames or more, and the engine logs each one at `debug`, so `RUST_LOG=egui_reactor=debug cargo run -p <example>` prints them; `egui-reactor-app` installs `env_logger` on native and eframe's `WebLogger` on the web when the app has not installed a logger of its own.
...
`<Suspense fallback={..}>children</Suspense>` (section 6, `egui-reactor-elements`) draws `fallback` instead of children if even one `use_future` inside is `Pending`. Rust has nothing like React's throw, so the child exits with `let Poll::Ready(x) = use_future(..) else { return };`, and the number of `Pending`s is counted with a counter in `Store`.
...
### Elements list (`egui-reactor-elements`)

All elements are written with `#[component]` and take `#[prop(default)] style: ItemStyle`. `rsx!` packs the layout attributes into `style`, and the paint attributes — `bg` `border` `radius` `shadow` `custom_shadow` `opacity` — with them, so every element can be a painted box. Everything, including the event enums, is re-exported from `egui_reactor_elements::prelude` (as in 3.6, the enum name is needed wherever `on_*` is used).

| Kind | Elements |
|---|---|
| Layout | `View` (`display` / `direction` / `wrap` / `justify` / `align` / `align_content` / `gap` / `cols`), `Text` (`size` / `color` / `strong` / `wrap`) |
| Widgets | `Button` (`enabled` / `label`, `on_click`), `Label` (`wrap`), `TextEdit` (`bind` / `multiline` / `hint` / `desired_width` / `rows` / `clear_on_submit`, `on_change` / `on_submit`), `Checkbox` (`bind` / `label`, `on_change`), `Slider<T: Numeric>` (`bind` / `range` / `label`, `on_change`), `ComboBox` (`bind` / `options` / `label`, `on_change`), `Image` (`source` / `fit` / `alt`), `Separator` (`vertical`) |
| Containers | `ScrollArea` (`horizontal` / `vertical` / `max_h`), `VirtualList` (`rows` / `row_h` / `row_w` / `render`, `on_scroll`), `Collapsing`, `Frame` (no props of its own; an `egui::Frame` built from `style` in `Ui` mode), `Window` (`title` / `open` / `resizable` / `default_pos` / `default_size`), `Overlay` (`anchor` / `offset` / `pos` / `order` / `constrain` / `top` / `fill`; sized by `w` / `h`, or by its children), `Panel` (`side`), `CentralPanel`, `Vertical`, `Horizontal`, `Grid` (`cols` / `striped`) + `Row` |
...
They live in `egui_reactor::layout`. taffy is a direct dependency, re-exported as `egui_reactor::taffy`.
...
```
egui-reactor/            core: View, Cx, Store, State, Handle, Dispatch, hooks, sweep, deferred queue, persistence, the layout engine (`engine/mod.rs` over taffy, `engine/lite.rs` for `<VirtualList>` rows)
egui-reactor-macros/     rsx! (based on rstml 0.13), #[component], #[hook]
egui-reactor-elements/   wrappers for egui widgets / containers. View / Text sit on taffy
egui-reactor-app/        run(Options, |_cx| rsx!{ <App/> }), fonts (CSS-style font chains resolved into egui's FontDefinitions). Wraps eframe and absorbs native / wasm / Android. The iOS runner also lives here
...
What `egui_reactor_app::run(Options, root)` does in one frame is the following.
...
- Visual parity is captured with the `snapshot` feature of `examples/gallery`. Draw the egui-reactor version and the raw egui version at the same size and compare against **the same single image**. The only remaining difference is text edges, absorbed with `SnapshotOptions::max_failed_pixels` (the value is in tasks/examples/plan.md section 7).
...
The following were confirmed with hand-written expansions without the macro (PR1, `crates/egui-reactor/tests/`). Every item has a test and is green. Where an assumption broke, the relevant section of this document has been updated.
...
| URL | What | Built by |
|---|---|---|
| `/`, `/guide/*`, `/reference/*`, `/examples/<name>` | The pages | VitePress into `site/.vitepress/dist` |
| `/embed/#<name>`, `/embed/#<name>/plain`, `?theme=dark` | One example, full-bleed, nothing else, in the page's theme | trunk, `examples/gallery/Trunk-embed.toml` |
| `/api/` | `cargo doc --no-deps` for the four public crates, plus a redirect to `egui_reactor/`, which rustdoc does not write | cargo |

- **The example pages are the gallery, redistributed.** The list is the sidebar, the code pane is the page body (with the egui-reactor / plain egui tabs where a plain version exists), and the running example is an iframe on `/embed/`. The native gallery (`cargo run -p gallery <name>`) is unchanged and stays the development tool.
- **One wasm for every example.** `examples/gallery/src/embed.rs` is a second bin in the gallery crate, so it reuses the example imports and `EXAMPLES`. It reads `location.hash` every pass, mounts `<Running>` under `key={name}` in a grow column, and registers a `hashchange` listener that calls `request_repaint` — egui repaints on input, and a hash change is not input. Unknown or missing names fall back to `EXAMPLES[0]`, and `plain` for an example without one falls back to the egui-reactor version, the same rule the gallery follows.
- **The example is drawn in the page's theme.** The page puts its appearance in the same hash (`#board?theme=dark`) and the embed calls `Context::set_theme` when it differs from what egui is in. Flipping the switch in the nav bar changes only the hash, which is a fragment navigation: the example keeps running and follows. Without the query egui follows the system, as it does natively. Examples with colours of their own (`board`, `notes`) read egui's theme rather than keeping one; none has a switch ([adr/app/0002](adr/app/0002-embed-follows-the-page-theme.md)).
- **The sources come from the example crates.** `site/lib/example-source.js` is a port of `gallery::shown_source` (drop the `//!` header, the `META` block and its import, `// gallery:hide` regions and the `#[cfg(test)]` tail, fold blank runs), used from a markdown-it block rule that expands `::: example-source <name>` into a `rust` fence, so Shiki highlights it at build time. The port and the Rust original have to agree or the site drifts from the gallery.
- **Old links keep working.** An inline script in the VitePress head rewrites `/#<name>` to `/examples/<name>.html` before the router hydrates, so the gallery's deep links land on the matching page.
- **`examples/gallery/tests/site.rs`** asserts that every `Meta` in `EXAMPLES` has a `site/examples/<name>.md` that includes its own source. Adding an example without its page fails in the same place adding it to the gallery is enforced.
- **CI** builds the site on every PR (`vitepress build` catches a dead link or a broken include) and builds the embed wasm alongside the examples; `cargo doc` is not run there.

...
| `/embed/#<name>`, `/embed/#<name>/plain` | One example, full-bleed, nothing else | trunk, `examples/gallery/Trunk-embed.toml` |
...
- **The sources come from the example crates.** `site/lib/example-source.js` is a port of `gallery::shown_source` (drop the `//!` header, the `META` block and its import, `// gallery:hide` regions, fold blank runs), used from a markdown-it block rule that expands `::: example-source <name>` into a `rust` fence, so Shiki highlights it at build time. The port and the Rust original have to agree or the site drifts from the gallery.
...
- **`examples/gallery/tests/site.rs`** asserts that every `Meta` in `EXAMPLES` has a `site/examples/<name>.md` that includes its own source, plus `shell.md`. Adding an example without its page fails in the same place adding it to the gallery is enforced.

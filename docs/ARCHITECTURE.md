...
- egui 標準の `<Vertical>` / `<Horizontal>` / `<Grid>` などのコンテナも leaf として残し、パフォーマンスが要る箇所の逃げ道にする。Taffy モードから呼ばれた場合、これらの egui-native なコンテナは 1 つの leaf として振る舞い、その中の子は Ui モードで描かれる。 `ScrollArea` だけは `leaf_fill`(3.1)で置く。与えられた空間を埋めるウィジェットなので、内容で測る leaf では最初のフレームの大きさに固定されてしまう。`<View>` の中の `ScrollArea` には `grow` か `h` を与える。**`leaf_fill` の max-content は「今のツリーのルート矩形の高さ」そのもの**(egui_taffy の measure が `infinite` をそう読み替える)なので、高さの確定していない `<View direction="column">` の中では `grow` も `basis={0}` も効かず、`ScrollArea` は「兄弟の残り」ではなく「窓の高さ全部」を要求する。ツールバーの下に `ScrollArea` を置いた画面はその分だけ窓より高くなり、ランナーのルート item style は `min_h: 100%`(7 章)で高さ自体は auto なので、はみ出しは切られずに下へ伸びる。確定した高さを与えるか、はみ出す下端に押すものを置かない構成にする(`examples/board` は列の `+ card` を `ScrollArea` の中の末尾に置いた。`docs/tasks/board/plan.md` 8.3)。

...
That left every scenario at one pass and the gap still at about 2x. A Time Profiler run said why: taffy's own algorithm costs nothing at idle (no sample lands in a `taffy::` frame) and egui_taffy's per-tree bookkeeping is 9%. About 80% is egui_taffy building one egui `Ui` per taffy node and a second one per leaf — **nine `Ui`s per row against four in plain egui**, a ratio of 2.25 against the measured 1.94x. That is egui_taffy's design, not a bug: its backgrounds, interactive containers and sticky scrolling need those `Ui`s. egui-reactor uses none of them, and egui_taffy was called from one file (`cx.rs`).

## Decision

egui_taffy was replaced by `crates/egui-reactor/src/engine.rs` (ARCHITECTURE section 6), which ports egui_taffy's measure function, node reuse rule and sweep, builds B and C in, and makes a container node a rect instead of a `Ui`:
...
- **Staying on egui_taffy and optimising inside it.** A Time Profiler run said the cost was neither taffy's algorithm nor a bug: about 80% was egui_taffy building one egui `Ui` per taffy node and a second per leaf, which its backgrounds, interactive containers and sticky scrolling need. egui-reactor uses none of those, so there was nothing to optimise away without changing its design.
